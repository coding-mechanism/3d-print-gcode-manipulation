...
const CHUNK_SIZE = 5 * 1024 * 1024; // 5MB in bytes
    export let gcodeArray = [];
      ...
      function preFilterGcode(gcode, sString){
         ...
         * */
      function reverseStringCharacterSearch(stringToSearch, characterToMatch, percentageToSearch = 100){
        ...
        let characterCountToStopSearch = Math.floor(((stringToSearch.length -1 ) * (percentageToSearch / 100)));

        for(let i = (stringToSearch.length - 1); i >= ((stringToSearch.length - 1) - characterCountToStopSearch); i--){
      ...
      // Function to handle the click event on individual lines
        if (event.target.tagName === 'TD') {
          ...
          const selectedCollection = document.getElementsByClassName('selected-line');
          const selectedCount = selectedCollection.length;
          let breakFunction = false;
          let isAlreadySelected = false;

          if(selectedCell.classList.length == 1 && selectedCell.classList[0] != "selected-line"){
            breakFunction = true;
          }
      ...
      if(!breakFunction && selectedCollection[0].value != prevSelection.value){
        prevSelection = selectedCell;
        selectedCollection[0].classList.remove('selected-line');
        selectedCell.classList.add('selected-line');
        breakFunction = true;
      }
      if(!breakFunction && selectedCollection[1].value != prevSelection.value){
        prevSelection = selectedCell;
    ...
    if(!breakFunction && isAlreadySelected && selectedCount == 2){
      if(!breakFunction && selectedCollection[0].value == selectedCell.value){
        prevSelection = selectedCollection[1];
        selectedCell.classList.remove('selected-line');
        breakFunction = true;
      }

      if(!breakFunction && selectedCollection[1].value == selectedCell.value){
